  ...
  return (
    ...
    >
      <h2>
        <button
          type="button"
          className={`flex items-center justify-between w-full px-6 py-4 font-semibold text-left dark:bg-dark dark:text-white ${className}`}
          onClick={() => setExpanded(!expanded)}
          aria-expanded={expanded}
          aria-controls={`acco-text-item`}
        >
          <div
            className="text-left rtl:text-right 2xl:text-xs text-[9px]"
            style={{ color: color }}
          >
            <h1>{title}</h1>
