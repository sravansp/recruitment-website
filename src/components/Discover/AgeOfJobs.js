...
const AgeOfJobs = () => {
  ...
  return (
    <div className="w-full h-80 overflow-y-auto joblistDash">
       <table className="flex flex-row flex-no-wrap w-full">
      <thead className="text-gray-500">
      {LiveJobsData.map((item, index) => (
        <tr className="flex flex-col mb-2 text-xs uppercase rounded-l-lg flex-no wrap sm:table-row sm:rounded-none sm:mb-0 bg-primaryalpha/10 dark:bg-white/20 sm:bg-transparent dark:sm:bg-transparent" key={index}>
      ...
      <tbody className="flex-1 sm:flex-none">
        {LiveJobsData.map((item, index) => (
          <tr key={index} className="flex flex-col text-xs sm:text-sm dark:text-white mb-2 flex-no wrap sm:table-row sm:mb-0 hover:bg-slate-600/5">
